...
def run_logistic_example(show_plots=True, return_data=False):
    """
    ...
    for i in range(kn + 1):
        ...
        if x_pos < 0.5:
            cv_mixed[i] = 1  # Convexe avant 0.5
        else:
            cv_mixed[i] = -1   # Concave après 0.5
    ...
    for tau in quantiles:
        ...
        # Toutes contraintes
        try:
            res = SplineCubicQuant(xtab, ytab, knots, tau, monot=monot_uniform, cv=cv_mixed, der3=der3_positive, solver=solver)
            results['results'][tau]['all'] = res
            print(f"✓ τ={tau}: Toutes contraintes OK")
        except Exception as e:
            print(f"✗ τ={tau}: Toutes contraintes {e}")
    ...
    if show_plots:
        ...
        tau_colors = {'none': 'blue', 'monot': 'green', 'cv_mixed': 'orange', 
                      'monot_cv': 'purple', 'all': 'red'}
        ...
        ax = axes[1, 1]
        ax.scatter(xtab, ytab, alpha=0.3, s=10, color='gray', label='Données')
        ax.set_title('5. Toutes les contraintes\n (avec dérivée 3e négative)')
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        ax.axvspan(0, 0.5, alpha=0.1, color='red')
        ax.axvspan(0.5, 1, alpha=0.1, color='blue')
...
def main():
    ...
    run_logistic_example(show_plots=True)
