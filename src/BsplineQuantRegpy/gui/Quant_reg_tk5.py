...
def get_project_paths():
    ...
    # current_dir est .../src/PysplineQuantReg/gui/
    # On remonte de 3 niveaux pour atteindre la racine
    project_root = current_dir
    for _ in range(3):
        project_root = os.path.dirname(project_root)
    
    # Vérifier que la racine est correcte en cherchant un fichier signature
    signature_files = ['pyproject.toml', 'setup.py', 'README.md', 'LICENSE']
    found = False
    ...
    if not found:
        # Si on n'a pas trouvé, essayer avec le répertoire parent du répertoire src
    ...
    # Ajouter les chemins à sys.path (en premier pour qu'ils soient prioritaires)
...
for path in [PROJECT_ROOT, SRC_PATH, EXAMPLES_PATH, SCRIPTS_PATH]:
    if path not in sys.path:
        sys.path.insert(0, path)
...
print(f"sys.path (premiers): {sys.path[:5]}")
...
class QuantileSplineApp:
    ...
    # ============ NOTEBOOK avec onglets ============
        ...
        self.check_modules()
        self.setup_menu()  # <-- AJOUTER CETTE LIGNE
   ...
   #     examples_menu.add_command(label="Données de test", command=self.generate_test_data)
   ...
   #     examples_menu.add_command(label="Comparaison des degrés", command=self.run_comparison_example)
        ...
        examples_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Exemples", menu=examples_menu)
        examples_menu.add_command(label="Données de test personnalisables", command=self.generate_test_data)
        #examples_menu.add_command(label="Données de test personnalisées", command=self.show_custom_test_dialog)
        #examples_menu.add_separator()
        examples_menu.add_command(label="📊 Données température", command=self.load_temperature_example)
        examples_menu.add_command(label="📈 Comparaison des degrés", command=self.run_comparison_example)
        examples_menu.add_command(label="🔬 Comparaison avec contraintes", command=self.run_comparison_with_constraints)
    def run_comparison_example(self):
     """Lance l'exemple de comparaison des degrés."""
     try:
        ...
        # Exécuter l'analyse (sans afficher les plots directement)
        # On va capturer les résultats et les afficher dans une fenêtre Tkinter
        import matplotlib.pyplot as plt
        from matplotlib.figure import Figure
        from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg
        
        # Créer une figure avec les résultats
        ...
        # Calculer et afficher les splines
        colors = ['blue', 'green', 'red', 'orange']
        deg_labels = ['Linéaire', 'Quadratique', 'Cubique', 'Quartique']
        tau = 0.5
        solver = 'CLARABEL'
        ...
        for d, color, label in zip([1, 2, 3, 4], colors, deg_labels):
            try:
                if d == 1:
                    res = SplineLinearQuant(x, y, knots, tau=tau, monot=0, solver=solver)
                elif d == 2:
                    res = SplineQuadraticQuant(x, y, knots, tau=tau, monot=0, cv=0, solver=solver)
                elif d == 3:
                    res = SplineCubicQuant(x, y, knots, tau=tau, monot=0, cv=0, der3=0, solver=solver)
                else:
                    res = SplineQuarticQuant(x, y, knots, tau=tau, monot=0, cv=0, d3=0, solver=solver)
        ...
        ax1.set_title('Sans contraintes')
        ax1.legend(fontsize='small')
        ax1.grid(True, alpha=0.3)
        
        # Ajouter les autres sous-graphiques...
        ...
        messagebox.showerror("Erreur", f"Impossible de charger l'exemple: {e}\nAssurez-vous que comparison_example.py est dans le dossier examples/")
     except Exception as e:
...
def load_example_module(module_name):
    """Charge un module d'exemple depuis le package."""
    try:
        # Essayer d'importer depuis le package
        import importlib
        return importlib.import_module(f'PysplineQuantReg.examples.{module_name}')
    except ImportError:
        # Essayer d'importer localement
        try:
            return importlib.import_module(module_name)
        except ImportError:
            raise ImportError(f"Module {module_name} non trouvé")
    ...
    def run_comparison_with_constraints(self):
        """Lance une comparaison détaillée avec différentes contraintes."""
        # Appeler directement la méthode existante
    ...
    def load_temperature_example(self):
     ...
     try:
        ...
        # Chercher le fichier temp.xls dans différents endroits
        possible_paths = [
    ...
    def process_dataframe(self, df):
        ...
        # CORRECTION : definir confirm comme fermeture capturant les variables
        def confirm():
            try:
    ...
    def run_regression(self):
        ...
        try:
            ...
            if self.degree == 1:
                        self.spline_result = SplineLinearQuant(
                            self.xtab, self.ytab, self.knots, tau, 
                            monot=monot, solver=solver
                        )
            elif self.degree == 2:
                        self.spline_result = SplineQuadraticQuant(
                            self.xtab, self.ytab, self.knots, tau, 
                            monot=monot, cv=cv, solver=solver
                        )
            elif self.degree == 3:
                        self.spline_result = SplineCubicQuant(
                            self.xtab, self.ytab, self.knots, tau, 
                            monot, cv, der3, solver=solver
                        )
            else:  # degree == 4
                        self.spline_result = SplineQuarticQuant(
                            self.xtab, self.ytab, self.knots, tau, 
                            monot, cv, der3, solver=solver
                        )
