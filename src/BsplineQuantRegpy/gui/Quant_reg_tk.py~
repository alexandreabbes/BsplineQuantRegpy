...
class QuantileSplineApp:
    ...
    def setup_menu(self):
        ...
        examples_menu.add_command(label="📈 Comparaison des degrés2", command=self.run_comparison_example2)
        examples_menu.add_command(label="📈 Basic Example", command=self.load_basic_example2)
            # Menu Scripts
        scripts_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Scripts", menu=scripts_menu)
        scripts_menu.add_command(label=" Exécuter example_basic.py", command=self.load_basic_example2)
        scripts_menu.add_command(label=" Exécuter comparison_example.py", command=self.run_comparison_example2)
    ...
    def process_dataframe(self, df):
        ...
        ttk.Combobox(dialog, textvariable=x_var, values=numeric_cols, state="readonly").grid(row=0, column=1, padx=5, pady=5)
        ...
        ttk.Combobox(dialog, textvariable=y_var, values=numeric_cols, state="readonly").grid(row=1, column=1, padx=5, pady=5)
    ...
    def load_example_basic2(self):
     ...
     try:
        ...
        possible_paths = [
            ...
            os.path.join(os.path.dirname(__file__), '..', '..', '..', 'examples', 'example_basic2.py'),
    ...
    def load_temperature_example(self):
        try:
            ...
            messagebox.showinfo("Succès", "Données de température chargées!\n\nNœuds aux années: 1880, 1889, 1900, 1910, 1930, 1940, 1965, 1992")
        except Exception as e:
            messagebox.showerror("Erreur", f"Impossible de charger l'exemple: {str(e)}")
            import traceback
            traceback.print_exc()
            
    def run_comparison_example2(self):
     """Lance l'exemple de comparaison des degrés."""
     try:
        # Ajouter le chemin examples à sys.path
        if EXAMPLES_PATH not in sys.path:
            sys.path.insert(0, EXAMPLES_PATH)
        
        # Vérifier que le fichier existe
        import os
        example_file = os.path.join(EXAMPLES_PATH, 'comparison_example.py')
        if not os.path.exists(example_file):
            messagebox.showerror("Erreur", 
                f"Fichier non trouvé: {example_file}\n\n"
                f"EXAMPLES_PATH = {EXAMPLES_PATH}")
            return
        
        # Importer le module dynamiquement
        import importlib.util
        spec = importlib.util.spec_from_file_location("comparison_example", example_file)
